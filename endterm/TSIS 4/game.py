...
def run_game(screen, settings, personal_best):
    ...
    # жыланның бастапқы орны және түсі
    snake = [[WIDTH//2, HEIGHT//2], [WIDTH//2 - BLOCK_SIZE, HEIGHT//2], [WIDTH//2 - 2*BLOCK_SIZE, HEIGHT//2]]
    dx, dy = BLOCK_SIZE, 0
    snake_color = tuple(settings["snake_color"])
    
    # бастапқы ұпай саны, деңгей, жылдамдық
    score = 0
    level = 1
    food_eaten_this_level = 0
    current_speed = BASE_SPEED
    
    # жыланға тиіспей, қабырғаларды кездейсоқ орналастыруға көмектесетін көмекші
    obstacles = []
        ...
        if level >= 3:
            ...
            for _ in range(num_obs):
                while True:
                    ...
                    # ортаңғы аймақты таза ұстау, сонда біз бірден жеңіліп қалмаймыз
    ...
    # тақтадан бұғатталмаған кездейсоқ орынды табу
    def get_random_pos():
        while True:
    ...
    # алғашқы тағамды және мүмкін уларды дайындау
    food = get_random_pos()
    food_type = "normal"
    food_timer = 0
